...
\begin{tabular}{|p{4cm}|p{8cm}|p{6cm}|}
	...
	$l^{*}=\dfrac{J_A}{m\,a}=\dfrac{J_M}{m\,x} = \dfrac{J_{S}}{m\cdot a}+a\quad a=\sqrt{\frac{J_s}{m}}$\\ 
	bei mehreren Elementen: \parbox{3cm}{
			$J_A = \sum J_{A_i}$\\
				 $m = \sum m_i$}\\ \\
	$J_A=J_S+\underbrace{m\,a^2}_\textrm{Punktmasse} \qquad J_M=J_S+m\,x^2$\\ \\
	...
	$l_{min}^* = 2\sqrt{\dfrac{J_S}{m}}$ wenn $a=x=a_{min}$\\$ T_{min} = 2\pi\sqrt{\frac{2}{g}\sqrt{\frac{J_s}{m}}}$
	\end{minipage} &
	\begin{minipage}[]{6cm}
...
\hline
\parbox{6cm}{
	\textbf{Unwuchterregung}\label{Unwucht}\\ \\
